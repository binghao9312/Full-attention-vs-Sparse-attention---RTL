...
The current compute core generates sparse QK scores. Scaling, softmax, and
weighted V accumulation are outside the current implementation scope and must
be re-evaluated before claiming a complete sparse-attention accelerator.
...
| `0x09` | `RESULT_K_IDX` | R | K index at result FIFO front |
| `0x0A` | `RESULT_SCORE_0` | R | Score bits `[7:0]` |
| `0x0B` | `RESULT_SCORE_1` | R | Score bits `[15:8]` |
| `0x0C` | `RESULT_SCORE_2` | R | Score bits `[23:16]` |
| `0x0D` | `RESULT_SCORE_3` | R | Score bits `[31:24]` |
...
| `0x18`-`0x1B` | `MAC_COUNT` | R | 32-bit MAC count |
...
## 8. Result Flow Control

The compute core can generate one 32-bit score per core cycle. The external
8-bit bus requires multiple cycles to read one result and therefore cannot
drain an unrestricted score stream directly.

The integration must include a result FIFO and valid/ready backpressure:

```text
Sparse core score_valid/score_ready
                 |
                 v
       Result FIFO: q_idx, k_idx, score
                 |
                 v
...
- A FIFO entry contains `{q_idx, k_idx, attention_score}`.
- The compute pipeline advances a result only on
  `score_valid && score_ready`.
- FIFO full deasserts `score_ready` and stalls pair generation and all score
  pipeline stages without changing ordering.
- FIFO front data remains stable while the host reads the six result bytes.
...
core score output   -> result FIFO push interface
...
## 12. T18 Implementation Requirements
...
- FIFO preserves Q/K/score ordering under continuous stalls.
- FIFO full stalls the complete score pipeline without dropping pairs.
...
- Compare every returned Q/K pair and QK score with the software golden model.
...
3. Result backpressure is implemented and proven lossless.
4. Current RTL synthesis reports area and timing using the selected T18 kit.
5. APR confirms core and pad-ring fit at the required utilization.
6. Post-layout timing passes with real I/O constraints and extracted parasitics.
7. Gate-level simulation, DRC, LVS, and required foundry checks pass.
